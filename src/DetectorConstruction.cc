...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  G4double checkR  = radius + thickwindow_r + mm;
  G4double worldR  = radius + thickwindow_r + cm;
  G4double targetZ = HistoManager::GetPointer()->Length()*0.5; 
  G4double checkZ  = targetZ + thickwindow_z + mm;
  G4double worldZ  = targetZ + thickwindow_z + cm;
 ...
 //
  // Window volume
  //
  G4Tubs* solidWindow = new G4Tubs("Window",0.,windowR,windowZ,0.,twopi);
...
void DetectorConstruction::SetTargetMaterial(const G4String& mat)
{
  ...
  G4String symbol;
  G4int nIso;
  D  = new G4Isotope("Deuteron", zz=1, nn=2, aa= 2.0141018*g/mole);
  elD = new G4Element("Deuterium",symbol="elD", nIso = 1);
  elD->AddIsotope(D, abundance=100.*perCent);
  LD2 = new G4Material("LiquidDeuterium", density= 0.169*g/cm3, nComponents=1, kStateLiquid, temp);
  ...
  G4Element* He3 = new G4Element("Helium3",  "He3", zz=2,  aa=3.0160293*g/mole);
	


  G4Material* He3_10amg = new G4Material( "He3_10amg",  density = 10.*0.1345*mg/cm3, nComponents=1 );
...
void DetectorConstruction::SetWindowMaterial(const G4String& mat)
{
 ...
 //He3 cell glass
  G4Element* Ba  = new G4Element("Barium",    "Ba", zz=56, aa=  137.327*g/mole);
  G4Element* O   = new G4Element("Oxygen",    "O",  zz=8,  aa=    16.00*g/mole);
  G4Element* Sr  = new G4Element("Strontium", "Sr", zz=38, aa=    87.62*g/mole);
  
  G4Material *BariumOxide = new G4Material("BariumOxide", density=5.72*g/cm3, nComponents=2);
  BariumOxide->AddElement(Ba, 1);
  BariumOxide->AddElement(O,  1);
  
  G4Material *StrontiumOxide = new G4Material("StrontiumOxide", density=4.7*g/cm3, nComponents=2);
  ...
  G4Material *Glass_GE180 = new G4Material("Glass_GE180", density=2.76*g/cm3, nComponents=5);
